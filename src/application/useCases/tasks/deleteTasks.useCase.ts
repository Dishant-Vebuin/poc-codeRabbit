...
export const deleteTasksUseCase = async (taskId: number, ownerId: number, tasksRepo: tasksPort, transaction: Transaction) => {
    const checkTaskId = await tasksRepo.getTasksById(taskId, transaction);
    if (!checkTaskId) {
        throw new Error(constants.ERROR_MESSAGE.TASKS_NOT_FOUND);
    }

    await tasksRepo.deleteTasks(taskId, ownerId, transaction);
}
